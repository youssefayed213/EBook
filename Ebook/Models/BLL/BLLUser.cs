...
using Ebook.Models.Entity.Product;
...
namespace Ebook.Models.BLL
{
    public static class BllUser
    {
        ...
        {
            return DalUser.AddPurchase(docId, userId, country);
        }
       ...
       public static JsonResponse AddToWishList(string docId, string userId)
        ...
        public static JsonResponse UpdatePurchaseApi(DocumentPurchaseVm model, string country, IToastNotification notification)
        {
            ...
            foreach (var iddoc in model.ListDoc)
            {
                if (!ValidatePurchase(iddoc.ToString(), model.UserId.ToString()).Success)
                {
                    message = AddPurchase(iddoc.ToString(), model.UserId.ToString(), country);
