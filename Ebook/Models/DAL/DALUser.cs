...
namespace Ebook.Models.DAL
{
    ...
    { 
        public static void CreateTable()
        {
            try
            {
                ...
                const string sql6 = "If not exists (select * from sysobjects where name = 'PurchaseHistory') " +
                                    "CREATE TABLE [dbo].[PurchaseHistory] ( [Id] bigint IDENTITY(1, 1) NOT NULL, [UserId] bigint NOT NULL, [DocumentId] bigint NOT NULL, [DocumentType] nvarchar(max) NOT NULL , [Country] nvarchar(max) NOT NULL, [Date] datetime DEFAULT GETDATE() NOT NULL  );";
        ...
        public static JsonResponse AddPurchase(string docId, string userId, string country)
        {
            var message = new JsonResponse
            {
                Success = false,
            ...
            try
            {
                ...
                {
                    connection.Open();
                    const string sql = "Insert into PurchaseHistory(UserId,DocumentId,DocumentType, Country) values (@UserId, @DocumentId, @DocumentType, @Country)";
                    using (var command = new SqlCommand(sql, connection))
                    {
                        command.CommandType = CommandType.Text;
